# Hello, I'm Gibran Reza Al Ghazzali 👋

I am an Electrical Engineering student at Telkom University ⚡. I specialize in bridging hardware and software, with a strong passion for embedded systems, IoT, robotics, and edge AI. I enjoy bringing hardware to life—from building real-time monitoring dashboards to developing AI-driven automation and remote-controlled robotics.

## 🚀 Top Projects

### 🏆 [SEMERBAK IoT - Innovillage 2024 (2nd Place)](https://github.com/bransazza/Semerbak-IoT.git)
An award-winning IoT system designed to tackle food insecurity. Features include an automated watering algorithm, real-time web dashboard integration, and complex electrical wiring.
- **Tech Stack:** ESP32, Arduino IDE, Firebase, HTML/JS/CSS, Fusion 360

### 🕷️ [KRSRI Search & Rescue Hexapod Robot](https://github.com/bransazza/KRSRI-ROS.git)
A six-legged Search and Rescue (SAR) robot developed for the KRSRI national robotics competition. Features autonomous navigation, object retrieval algorithms, and real-time image processing.
- **Tech Stack:** ROS/ROS2, STM32, Python, C++, OpenCV, Ubuntu

### ♻️ [AI-Powered Sorting Conveyor](https://github.com/bransazza/Digital-Conveyor-Xiao-Computer-Vision.git)
A digital conveyor system that automates object classification and waste sorting using Machine Learning at the edge.
- **Tech Stack:** XIAO ESP32S3, STM32, Edge Impulse (TinyML), PlatformIO, EasyEDA

*Note: Click on the project titles to view the repositories!*

## ✨ Technical Skills

- **Programming:** C, C++, Python, JavaScript, HTML/CSS
- **Hardware & Microcontrollers:** STM32, ESP32, Arduino, PCB Design (EasyEDA, Eagle), PLC Omron, Sensor Integration, Wiring & Panel Control
- **Robotics & AI:** ROS/ROS2, Computer Vision (OpenCV), Edge Impulse, Control Systems (PID), Hexapod Kinematics
- **Software & Tools:** VS Code, STM32CubeIDE, Arduino IDE, Git, Firebase, CX-Programmer, Fusion 360, Ubuntu

## 📊 GitHub Stats

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=bransazza&show_icons=true&theme=radical" alt="Bransazza's GitHub stats" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=bransazza&layout=compact&theme=radical" alt="Top Langs" />
</div>

## 📫 How to Reach Me

- **LinkedIn:** [Gibran Reza Al Ghazzali](https://www.linkedin.com/in/gibran-reza-al-ghazzali-1380a0322)
- **Email:** [gibranreza0@gmail.com](mailto:gibranreza0@gmail.com)
- **Portfolio:** [Check out my full portfolio here!](https://portogibran.vercel.app/) 


